...
export default function CreateUsers() {
    ...
    const getSectors = async () => {
        try {
            ...
            const response2 = await axios.get('/api/adminka/updateStanock');
            setStanock(response2.data.sort((a: Sector, b: Sector) => a.id - b.id));
    ...
    const columns: GridColDef[] = [
        ...
        {
            field: 'stanokName',
            headerName: 'Связь с станком',
            width: 200,
            renderCell: (params) => {
                const machines = params.row?.machines;
                if (machines && machines.length > 0) {
                    return machines.map((machine) => machine.name).join(', ');
                }
                return 'Без станков';
            },
        },
        {
            field: 'masterStanokName',
            headerName: 'Доп связь с мастером',
            width: 200,
            renderCell: (params) => {
                const masterMachines = params.row?.masterMachines;
                if (masterMachines && masterMachines.length > 0) {
                    return masterMachines.map((masterMachine) => masterMachine.name).join(', ');
                }
                return 'Без станков';
            },
        },
