...
export async function PUT(req: NextRequest) {
    try {
        const {sectorId, name, unitId, sectionId} = await req.json();
        console.log(name);
        // console.log(formData);
        
        // Обновляем запись в базе данных с помощью Prisma
        const updatedSectors = await prisma.putAdminStanock(sectorId, name, unitId,sectionId);
...
export async function DELETE(req: NextRequest) {
    try {
        ...
        // Обновляем запись в базе данных с помощью Prisma
        const updatedSectors = await prisma.delAdminStanock(sectorId);
